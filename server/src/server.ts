...
app.post('/api/auth/login', (req, res) => {
  ...
  if (!user) {
    return res.status(401).json({ error: `User "${identifier}" not found. Please register or use one of the demo accounts.` });
  }
...
app.post('/api/admin/drivers', authenticate, requireRole('admin'), (req: AuthRequest, res) => {
  ...
  db.createUser(newDriver);
  const { password: _, ...driverWithoutPw } = newDriver;
  res.status(201).json(driverWithoutPw);
});

// Admin: Edit Driver Details
app.put('/api/admin/drivers/:id', authenticate, requireRole('admin'), (req: AuthRequest, res) => {
  const { id } = req.params;
...
// Admin: Delete Driver
app.delete('/api/admin/drivers/:id', authenticate, requireRole('admin'), (req: AuthRequest, res) => {
  const { id } = req.params;
  const driver = db.getUserById(id);
